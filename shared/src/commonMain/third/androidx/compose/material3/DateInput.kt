...
internal fun DateInputContent(
    ...
    val labelText = getString2(string = Strings2.DateInputLabel)
    DateInputTextField(
        modifier = Modifier
            .fillMaxWidth()
...
internal class DateInputValidator(
    ...
    fun validate(
        ...
        if (dateToValidate == null) {
            return errorDatePattern.formatString(dateInputFormat.patternWithDelimiters.uppercase())
        }
        // Check that the date is within the valid range of years.
        if (!yearRange.contains(dateToValidate.year)) {
            return errorDateOutOfYearRange.formatString(
        ...
        with(selectableDates) {
            if (!isSelectableYear(dateToValidate.year) ||
                ...
                return errorInvalidNotAllowed.formatString(
